...
import { DomainChallengePage as EthicsDomainChallengePage } from "@/pages/ethics/DomainChallengePage";
import { DomainIntroPage as EthicsDomainIntroPage } from "@/pages/ethics/DomainIntroPage";
import { LessonPage as EthicsLessonPage } from "@/pages/ethics/LessonPage";
import { ModuleListPage as EthicsModuleListPage } from "@/pages/ethics/ModuleListPage";
import { QuizPage as EthicsQuizPage } from "@/pages/ethics/QuizPage";
import { Simulation2Page as EthicsSimulation2Page } from "@/pages/ethics/Simulation2Page";
import { SimulationPage as EthicsSimulationPage } from "@/pages/ethics/SimulationPage";
import { WildPage as EthicsWildPage } from "@/pages/ethics/WildPage";
...
// ─── Ethics Routes ───────────────────────────────────────────────────────────
const ethicsDomainRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics",
  component: EthicsDomainIntroPage,
});

const ethicsModuleListRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/modules",
  component: EthicsModuleListPage,
});

const ethicsLessonRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/module/$moduleId/lesson",
  component: () => {
    const { moduleId } = ethicsLessonRoute.useParams();
    const id = Number.parseInt(moduleId, 10);
    if (Number.isNaN(id) || id < 1 || id > 4) return null;
    return <EthicsLessonPage moduleId={id} />;
  },
});

const ethicsSimulationRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/module/$moduleId/simulation",
  component: () => {
    const { moduleId } = ethicsSimulationRoute.useParams();
    const id = Number.parseInt(moduleId, 10);
    if (Number.isNaN(id) || id < 1 || id > 4) return null;
    return <EthicsSimulationPage moduleId={id} />;
  },
});

const ethicsSimulation2Route = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/module/$moduleId/simulation/2",
  component: () => {
    const { moduleId } = ethicsSimulation2Route.useParams();
    const id = Number.parseInt(moduleId, 10);
    if (Number.isNaN(id) || id < 1 || id > 4) return null;
    return <EthicsSimulation2Page moduleId={id} />;
  },
});

const ethicsQuizRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/module/$moduleId/quiz",
  component: () => {
    const { moduleId } = ethicsQuizRoute.useParams();
    const id = Number.parseInt(moduleId, 10);
    if (Number.isNaN(id) || id < 1 || id > 4) return null;
    return <EthicsQuizPage moduleId={id} />;
  },
});

const ethicsWildRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/module/$moduleId/wild",
  component: () => {
    const { moduleId } = ethicsWildRoute.useParams();
    const id = Number.parseInt(moduleId, 10);
    if (Number.isNaN(id) || id < 1 || id > 4) return null;
    return <EthicsWildPage moduleId={id} />;
  },
});

const ethicsChallengeRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/domain/ethics/challenge",
  component: EthicsDomainChallengePage,
});
...
const routeTree = rootRoute.addChildren([
  ...
  // Ethics
  ethicsDomainRoute,
  ethicsModuleListRoute,
  ethicsLessonRoute,
  ethicsSimulationRoute,
  ethicsSimulation2Route,
  ethicsQuizRoute,
  ethicsWildRoute,
  ethicsChallengeRoute,
]);
