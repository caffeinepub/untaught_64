      ...
      >
        <div>
          ...
          <h3 className="font-display text-xl font-semibold text-foreground mb-3">
            The Laptop Negotiation
          </h3>
          <p className="text-sm text-foreground leading-relaxed mb-3">
            You're selling a second-hand laptop and have agreed to meet a buyer.
            The laptop is in good condition and both of you know the market
            value is roughly £200–250.
          </p>
          <p className="text-sm text-muted-foreground leading-relaxed mb-3">
            <strong className="text-foreground/80">What's at stake:</strong>{" "}
            You'll watch the same negotiation play out twice — once where the
            buyer names a number first, and once where you do. The difference in
            the final price will show exactly how much the opening anchor
            matters.
          </p>
          <p className="text-sm text-muted-foreground leading-relaxed">
            <strong className="text-foreground/80">Your role:</strong> Choose a
            scenario and observe how the first number said shapes every number
            that follows.
          </p>
        </div>
        <button
          ...
          data-ocid="be_sim2.start.button"
      ...
      <div className="flex gap-3">
        {(["A", "B"] as const).map((s) => (
          <button
            ...
            className="flex-1 rounded-lg border px-4 py-2.5 text-sm font-medium transition-all min-h-[44px]"
            style={{
              borderColor:
                active === s
                  ? `oklch(${AMBER} / 0.6)`
