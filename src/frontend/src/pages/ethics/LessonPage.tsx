...
const ERR = "0.72 0.2 29";
const LESSONS: Lesson[] = [M1_LESSON, M2_LESSON, M3_LESSON, M4_LESSON];

const SECTION_TYPE_CONFIG = {
  concept: { icon: BookOpen, label: "Key Concept" },
  mechanism: { icon: Settings, label: "How It Works" },
  warning: { icon: AlertTriangle, label: "Watch Out" },
  example: { icon: Eye, label: "Real Example" },
  trap: { icon: Zap, label: "The Trap" },
  fix: { icon: CheckCircle, label: "The Fix" },
} as const;
...
function SectionCard({
  ...
  const handleNext = () => {
    if (isLastSection && !showSummary) setShowSummary(true);
    else onNext();
  };
    ...
    >
      ...
      <div className="px-5 py-5 space-y-4">
        ...
        {section.check && isActive && (
          <div className="mt-5 rounded-lg bg-muted/25 p-4 sm:p-5">
            <p
            ...
            <div className="space-y-2">
              {section.check.options.map((option, optIdx) => {
                ...
                if (isAnswered) {
                  if (isWrongSelected) {
                    optionStyle = {
                      borderColor: `oklch(${ERR})`,
                      backgroundColor: `oklch(${ERR} / 0.12)`,
                      color: `oklch(${ERR})`,
                    };
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      if (selectedOption === null) setSelectedOption(optIdx);
                    }}
                    disabled={isAnswered}
                    className={[
                      "w-full text-left text-sm px-4 py-3 rounded-lg border transition-all duration-200 min-h-[44px] focus-visible:outline-none focus-visible:ring-2",
                      optionClass,
                    ].join(" ")}
                    style={optionStyle}
                  >
              ...
              {selectedOption !== null && (
                ...
                >
                  <p
                    ...
                    style={{ color: `oklch(${isCorrect ? GREEN : ERR})` }}
                  >
...
export function LessonPage({ moduleId }: LessonPageProps) {
  ...
  const handleNext = (sectionIdx: number) => {
    ...
    if (isLast) {
      if (!isRead) {
        ...
        void navigate({ to: `/domain/ethics/module/${moduleId}/simulation` });
      }
    } else {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12">
      <div className="mb-10">
        <BackButton to="/domain/ethics/modules" label="All modules" />
      </div>
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className="flex-1 h-1.5 rounded-full bg-border/30 overflow-hidden">
          <motion.div
            ...
            style={{ backgroundColor: `oklch(${RED})` }}
      ...
      <div className="mb-8 rounded-lg border border-border/30 bg-muted/20 px-4 py-3">
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          Comprehension checks are here to help you think, not to test you.
        </p>
      </div>
