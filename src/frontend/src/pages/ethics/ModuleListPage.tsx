...
export function ModuleListPage() {
  ...
  const totalModules = ETHICS_MODULES.length;
  const completedCount = ETHICS_MODULES.filter((m) =>
    completedModules.includes(`ethics-module-${m.id}`),
  ).length;
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12">
      <div className="mb-10">
        <BackButton to="/domain/ethics" label="Domain intro" />
      </div>
      ...
      <div className="space-y-3">
        {ETHICS_MODULES.map((module, i) => {
          const moduleKey = `ethics-module-${module.id}`;
