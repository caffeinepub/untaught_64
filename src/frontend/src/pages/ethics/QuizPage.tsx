...
import type { QuizQuestion } from "@/data/ethics";
...
export function QuizPage({ moduleId }: QuizPageProps) {
  ...
  const handleSubmit = () => {
    ...
    submitQuiz(
      ...
      {
        onSuccess: () => {
          if (passed) markModuleComplete(`ethics-module-${moduleId}`);
        },
      },
    );
  };
  ...
  const handleNext = () =>
    void navigate({ to: `/domain/ethics/module/${moduleId}/wild` });
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8 sm:py-12">
      <div className="mb-10">
        <BackButton
          to={`/domain/ethics/module/${moduleId}/simulation/2`}
        ...
        <div
          ...
          style={{
            backgroundColor: results.passed
              ? "oklch(0.65 0.18 145 / 0.1)"
              : `oklch(${RED} / 0.1)`,
            border: `1px solid ${results.passed ? "oklch(0.65 0.18 145 / 0.3)" : `oklch(${RED} / 0.3)`}`,
          }}
        >
          <p
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {results.passed
              ? "Great work. Continue to see this in the real world."
              : "Review the explanations above, then continue when ready."}
          </p>
        </div>
      )}
